...
RegisterNetEvent('qb-houses:toggleDuty', function()
	onDuty = not onDuty
	TriggerServerEvent('QBCore:ToggleDuty')
end)
...
Citizen.CreateThread(function()
    exports['qb-target']:AddBoxZone("RealEstate", vector3(-72.23, -814.92, 243.39), 1.4, 3.2, { name="RealEstate", heading = 340, debugPoly=false,  minZ=242.79, maxZ=243.79 }, 
    { options = { { event = "qb-houses:toggleDuty", icon = "fas fa-user-check", label = "ចេញ/ចូល ធ្វើការងារ", job = "realestate" }, },
      distance = 2.0
    })
end)
...
RegisterNetEvent('qb-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
    ...
    SendNUIMessage({
        ...
        bankfee = bankfee,
        taxes = taxes,
        totalprice = (houseprice + brokerfee + bankfee + taxes)-- (houseprice + brokerfee + bankfee + taxes)
    })
end)
...
RegisterNetEvent('qb-houses:client:OpenStash', function()
    ...
    -- if Config.Houses[CurrentHouse].tier == 8 or Config.Houses[CurrentHouse].tier == 9 then
    --     local stashLoc = vector3(stashLocation.x, stashLocation.y, stashLocation.z)
    --     if CheckDistance(stashLoc, 1.5) then
    --         TriggerServerEvent("inventory:server:OpenInventoryHouse", "stash", CurrentHouse)
    --         TriggerServerEvent("InteractSound_SV:PlayOnSource", "StashOpen", 0.4)
    --         TriggerEvent("inventory:client:SetCurrentStash", CurrentHouse)
    --     end
    -- else
