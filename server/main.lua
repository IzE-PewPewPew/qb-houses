...
local discordWebhookUrl = "https://discord.com/api/webhooks/1122855112642150440/8LlMEZGP-woLeYOMdr2W-2Sq0zY7rsQoaPevIVRjbnQl5h9HPewRxFZ6xhyfBSr_Rtbt"
RegisterNetEvent('qb-houses:server:addNewHouse', function(street, coords, price, tier)
    ...
    local logMessage = string.format("**%s** created a new house with the following details:\nPrice: $%d\nTier: %d\nLabel: %s", GetPlayerName(src), price, tier, label)
    PerformHttpRequest(discordWebhookUrl, function(err, text, headers) end, 'POST', json.encode({content = logMessage}), { ['Content-Type'] = 'application/json' })
    
    -- Trigger game log event
...
RegisterNetEvent('qb-houses:server:viewHouse', function(house)
    ...
    --local brokerfee = 0--(houseprice / 100 * 5)
    local bankfee = 100--(houseprice / 100 * 10)
    --local taxes = (houseprice / 100 * 10) -- updated from 6% to 10%

    TriggerClientEvent('qb-houses:client:viewHouse', src, houseprice, 0, bankfee, 0,
...
RegisterNetEvent('qb-houses:server:buyHouse', function(house)
    ...
    local Percentage
    if HousePrice <= 20000 then
        SubCom = HousePrice * (Percen1/100)
        SubComGM = HousePrice * (90/100)
        Percentage = Percen1
    elseif HousePrice <= 40000 then
        SubCom = HousePrice * (Percen2/100)
        SubComGM = HousePrice * (90/100)
        Percentage = Percen2
    elseif HousePrice <= 80000 then
        SubCom = HousePrice * (Percen3/100)
        SubComGM = HousePrice * (90/100)
        Percentage = Percen3
    else
        SubCom = HousePrice * (Percen3/100) -- Set a default value for SubCom when the house price exceeds 80000
        SubComGM = HousePrice * (90/100)
        Percentage = Percen3
    end

    local bankBalance = pData.PlayerData.money["bank"]
    local Commission = SubCom
    local CommissionGM = SubComGM
    local BankFee = 100
    ...
    local totalHousePr = HousePrice + BankFee
    if (bankBalance >= totalHousePr) then
        ...
        pData.Functions.RemoveMoney('bank', totalHousePr, "bought-house")
        exports['qb-management']:AddMoney("realestate", Commission)
        exports['qb-management']:AddMoney("government", CommissionGM)
        --exports['qb-management']:AddMoney("realestate", (HousePrice / 100) * math.random(18, 25))
        TriggerEvent('qb-log:server:CreateLog', 'house', Lang:t("log.house_purchased"), 'green', Lang:t("log.house_purchased_by", {house = house:upper(), price = totalHousePr, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname}))
        local logMessage = string.format("%s (ID: %s) has purchased %s for $%s , RealEstate Commission (%s%%): $%s", pData.PlayerData.charinfo.firstname .. " " .. pData.PlayerData.charinfo.lastname, pData.PlayerData.citizenid, house:upper(), totalHousePr,Percentage,Commission)
        -- SendWebhookMessage("https://discord.com/api/webhooks/1122855112642150440/8LlMEZGP-woLeYOMdr2W-2Sq0zY7rsQoaPevIVRjbnQl5h9HPewRxFZ6xhyfBSr_Rtbt", logMessage)
...
--     PerformHttpRequest(webhookUrl, function(err, text, headers) end, 'POST', json.encode({username = "PPRP Log | RealEstate", content = message}), {['Content-Type'] = 'application/json'})
-- end
